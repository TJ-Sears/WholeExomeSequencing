...
do
echo $i
run_strelka $strelka_dir $strelka_exome_config $reference_fasta $working_dir $i
done
}
...
import_cappseq_calls() {  #incomplete
if [ -a ../../analysis/tumor-samtools-deduped/filtered-somatic-mutations.annotated.txt ] ####replace dir structure; changed to a vcf without annotation
	then
	grep $tag $starting_dir/analysis/tumor-samtools-deduped/filtered-somatic-mutations.annotated.txt > $working_dir/$tag/$tag.cappseq.variants ####replace dir structure
###Process cappseq with VAF for convert2annovar 
fi
}
...
for i in $tumor_samples
do
	$scripts_dir/process_mouse_variants.sh $i $working_dir $annovar_dir $variant_dir $scripts_dir
done
}
...
protocol=refGene,cytoBand,snp142Common
operation=g,r,f

perl $annovar_dir/annotate_variation.pl -buildver mm10 -downdb refGene $db/
perl $annovar_dir/annotate_variation.pl -buildver mm10 -downdb cytoBand $db/
perl $annovar_dir/annotate_variation.pl -buildver mm10 -downdb snp142Common $db/
perl $annovar_dir/annotate_variation.pl --buildver mm10 --downdb seq $db/mm10_seq

perl $annovar_dir/retrieve_seq_from_fasta.pl $db/mm10_refGene.txt -seqdir $db/mm10_seq -format refGene -outfile $db/mm10_refGeneMrna.fa

perl $annovar_dir/table_annovar.pl $avinputfile $db/ --buildver mm10 -protocol $protocol -operation $operation -nastring "." --otherinfo

perl $annovar_dir/table_annovar.pl $consensusavinputfile $db/ --buildver mm10 -protocol $protocol -operation $operation -nastring "." --otherinfo

perl $annovar_dir/table_annovar.pl $germlineavinputfile $db/ --buildver mm10 -protocol $protocol -operation $operation -nastring "." --otherinfo

perl $annovar_dir/coding_change.pl --includesnp $consensusavinputfile.refGene.exonic_variant_function $db/mm10_refGene.txt $db/mm10_refGeneMrna.fa > $working_dir/$tag/$tag.codingchange

perl $annovar_dir/coding_change.pl --includesnp $avinputfile.refGene.exonic_variant_function $db/mm10_refGene.txt $db/mm10_refGeneMrna.fa > $working_dir/$tag/$tag.merged.codingchange

perl $annovar_dir/coding_change.pl --includesnp $germlineavinputfile.refGene.exonic_variant_function $db/mm10_refGene.txt $db/mm10_refGeneMrna.fa > $working_dir/$tag/$tag.germline.codingchange

cp $working_dir/$tag/$tag.*.avinput.mm10_multianno.txt $variant_dir/
}
...
for tag in $tumor_samples
	...
	$scripts_dir/MSK_process_annovar_output.sh $working_dir/$tag/$tag.merged.avinput.hg19_multianno.txt $working_dir/$tag/$tag.merged.codingchange $tag
	make_fasta $tag $tag.variants.fasta mut

	$scripts_dir/MSK_process_annovar_output.sh $working_dir/$tag/$tag.germline.avinput.hg19_multianno.txt $working_dir/$tag/$tag.germline.codingchange $tag
	make_fasta $tag $tag.germline_snps.fasta snp

	$scripts_dir/MSK_process_annovar_output.sh $working_dir/$tag/$tag.consensus.avinput.hg19_multianno.txt $working_dir/$tag/$tag.codingchange $tag
	make_fasta $tag $tag.stringent_variants.fasta mut	
done
}
